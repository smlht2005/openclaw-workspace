...
| 目錄 | 職責 |
|------|------|
| `brain/` | **知識庫** - 整合手冊、技術文檔、學習筆記 |
| `scripts/` | **腳本** - 工具腳本、自動化程式 |
| `skills/` | **技能** - OpenClaw 技能模組 |
| `docs/` | **暫存文檔** - 臨時文檔 (最終需整合到 brain/) |
| `memory/` | **日誌** - 每日工作日誌 |
| `templates/` | **範本** - 報告、任務範本 |

### 知識庫原則
1. **知識收集** → 統一存放在 `brain/` 目錄
2. **整合優化** → 定期將 `docs/` 整合到 `brain/`
3. **清理原則** → `docs/` 只存放暫時性文檔

### 檔案命名
- 知識庫: `brain/<主題>-handbook.md`
- 腳本: `scripts/<類別>/<名稱>.py`
...
| 方式 | 說明 | 適用場景 |
|------|------|----------|
| Squash | 壓縮成單一 commit | 功能分支，保持歷史簡潔 |
| Merge | 保留所有 commits | 需要保留完整歷史 |
| Rebase | 線性歷史 | 需要乾淨的 commit 歷史 |
...
### PR/Merge 自動化
- **PR 建立** → 自動記錄到 `todos/all-todos.md` 狀態為 🔍 Review
- **Merge 完成** → 自動更新狀態為 ✅ Done
